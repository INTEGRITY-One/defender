...
test "$SENDMETRIC" -eq 1 && aws cloudwatch put-metric-data --metric-name ApplicationDownCount --namespace "Defender.ApplicationMetrics" --value $DOWNSTATUS --timestamp $NEWLASTUPDATE

# pages served metric
PAGESSERVED=`cat $INTERVALLOG | wc -l`
echo "Pages Served in this interval: $PAGESSERVED"
    test "$SENDMETRIC" -eq 1 && aws cloudwatch put-metric-data --metric-name PageViewCount --namespace "Defender.ApplicationMetrics" --value $PAGESSERVED --timestamp $NEWLASTUPDATE
...
echo "Users Served in this interval: $USERSSERVED"
    test "$SENDMETRIC" -eq 1 && aws cloudwatch put-metric-data --metric-name UserCount --namespace "Defender.ApplicationMetrics" --value $USERSSERVED --timestamp $NEWLASTUPDATE
...
echo "Application errors in this interval: $ERRORPAGES"
    test "$SENDMETRIC" -eq 1 && aws cloudwatch put-metric-data --metric-name PageErrorCount --namespace "Defender.ApplicationMetrics" --value $ERRORPAGES --timestamp $NEWLASTUPDATE
...
test "$SENDMETRIC" -eq 1 && aws cloudwatch put-metric-data --metric-name AverageResponseTime --namespace "Defender.ApplicationMetrics" --value $AVGRESPONSETIME --timestamp $NEWLASTUPDATE
...
test "$SENDMETRIC" -eq 1 && aws cloudwatch put-metric-data --metric-name MaxResponseTime --namespace "Defender.ApplicationMetrics" --value $MAXRESPONSETIME --timestamp $NEWLASTUPDATE
